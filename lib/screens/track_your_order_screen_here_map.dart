...
import 'package:mealup/utils/widgets/timeline.dart';
...
// Map storing polylines created by connecting two points
  ...
  void initState() {
    ...
    getUserLocation().whenComplete(() {
      _add(widget.currentLat, widget.currentLong, 'assets/ic_green_marker.svg', MarkerId('current'));
      _add(widget.shopLat, widget.shopLong, 'assets/ic_marker.svg', MarkerId('shop'));
      _add(widget.shopLat, widget.shopLong, 'assets/ic_driver_icon.svg', MarkerId('driver'));
  ...
  Future<BaseModel<TrackingModel>> callStartTracking(int? orderId) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          setMarker(counter, double.parse(response.data!.driverLat!), double.parse(response.data!.driverLang!));
        });
      } else {
        setMarker(counter, double.parse(response.data!.driverLat!), double.parse(response.data!.driverLang!));
      }
  ...
  setMarker(int counter, double lat, double lang) async {
    ...
    BitmapDescriptor bitmapDescriptorDriver = await _bitmapDescriptorFromSvgAsset(context, 'assets/ic_driver_icon.svg');
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: CustomAppBarWithButton(
          title: '',
          buttonText: Languages.of(context)!.labelViewOrderDetails,
                                            ...
                                            InkWell(
                                              ...
                                              child: Padding(
                                                ...
                                                child: Container(
                                                  ...
                                                  decoration: BoxDecoration(shape: BoxShape.circle, color: Constants.colorTheme),
                                                ),
                                              ),
                                            )
                                          ],
